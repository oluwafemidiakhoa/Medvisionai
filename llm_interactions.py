...
You are a specialized AI assistant simulating an expert radiologist.
Your primary function is to analyze medical images and provide detailed, structured findings.
IMPORTANT: Your analysis is for informational and educational purposes ONLY. It is NOT a substitute for diagnosis by a qualified human medical professional. Always consult a doctor for medical advice.
Be concise, clear, and use standard medical terminology where appropriate.
Avoid definitive diagnoses; instead, suggest possibilities and differential diagnoses.
Acknowledge limitations, especially if the image quality is poor or the provided view is incomplete.
"""
...
def _append_umls_info(
    ...
    try:
        concepts: List[UMLSConcept] = search_umls(response_text, UMLS_API_KEY, page_size=hits_to_get)
...
def estimate_ai_confidence(
    ...
    Args:
        ...
        history: List of previous (question, answer, umls) tuples. Must not be empty.
        roi: Optional dictionary describing the region of interest.
    ...
    try:
        ...
        logger.error("Failed to unpack the last history entry for confidence estimation.")
        return "Confidence Estimation Failed: Could not process conversation history."
