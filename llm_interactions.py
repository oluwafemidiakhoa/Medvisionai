...
import logging
import streamlit as st  # Assumes Streamlit for secrets management
...
_BASE_ROLE_PROMPT = "Base Role Prompt goes here."
HISTORY_TURNS_FOR_CONTEXT = 5

# Dummy definitions for additional prompt templates
INITIAL_ANALYSIS_PROMPT_TEMPLATE = f"""{_BASE_ROLE_PROMPT}

Initial Analysis:
ROI Information: {{roi_info}}
"""
...
def query_gemini_vision(image: Image.Image, text_prompt: str) -> Tuple[Optional[str], bool]:
    ...
    try:
        ...
        if prompt_feedback and prompt_feedback.get('blockReason'):
            ...
            blocked_cats = [
                sr.get('category', 'UNKNOWN')
                for sr in prompt_feedback.get('safetyRatings', [])
                if sr.get('blocked')
            ]
        ...
        if finish_reason != "STOP":
            ...
            if blocked_cats:
                warn_msg += f" Blocked safety categories: {blocked_cats}"
        ...
        if status_code in [401, 403]:
            return f"API Error: Authentication/Permission Failed ({status_code}). Check API Key.", False
        if status_code == 429:
...
def _format_roi_info(roi: Optional[Dict]) -> str:
    ...
    return "Analysis applies to the entire image (no specific ROI highlighted)."
...
def run_multimodal_qa(
    ...
    """
    Handles Q&A, returning a tuple containing (answer_text, success_flag).
    """
...
def run_disease_analysis(image: Image.Image, disease: str, roi: Optional[Dict] = None) -> str:
    """
    Performs disease-focused analysis, returning a string result or an error message.
    """
...
def run_llm_self_assessment(
    ...
    try:
        ...
        # If last answer was an error, provide direct feedback
        lower_last_a = last_a.lower()
        if any(err in lower_last_a for err in ["error:", "failed:", "blocked", "unavailable", "could not"]):
            logger.warning(f"Last interaction was an error: '{last_a[:100]}...'")
            return (
    ...
    # Prepare and run the self-assessment prompt
    roi_info = _format_roi_info(roi)
    ...
    if success and response_text:
        if "## 1. Clarity" in response_text and "## 5. Overall Assessment" in response_text:
            ...
            logger.warning("Self-assessment response did not match expected Markdown format.")
            return f"**{action_name} Result (Format Warning):**\n\n{response_text}"
    else:
        return f"**{action_name} Failed:** {response_text or 'Unknown API error.'}"

# --- End of llm_interactions.py ---
