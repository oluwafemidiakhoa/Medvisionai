# main_app.py (Revision - Revert Aggressive RGB, Keep Patch, Restore Canvas Params)
                ...
                try:
                    ...
                    if st.session_state.is_dicom:
                        ...
                        if ds:
                            ...
                            if isinstance(temp_display_image, Image.Image) and isinstance(temp_processed_image, Image.Image):
                                # Allow L/RGB/RGBA initially, conversion handled later
                    ...
                    # --- Standard Image Processing Branch ---
                        ...
                        try:
                            ...
                            # Convert display image only if problematic mode (e.g., P, CMYK)
                            # Allow L, RGB, RGBA initially
                            ...
                            # Prepare processed image: Convert P/RGBA to RGB, keep L as is for analysis flexibility
                ...
                if needs_update:
                    ...
                    with st.spinner("Applying new Window/Level settings..."):
                         ...
                         if isinstance(new_display_img, Image.Image):
                              # Assign directly, preparation for canvas happens later
...
with col1:
    ...
    if isinstance(display_img_object, Image.Image):
        # Add explicit warning about checking browser console
        ...
        bg_image_pil = None # Initialize variable for canvas background
        try:
            # --- Prepare image specifically for Canvas (flexible conversion) ---
            logger.debug(f"Canvas Prep: Starting with display_image mode: {display_img_object.mode}")
            # Check mode and convert if necessary for canvas compatibility (RGB/RGBA preferred)
            if display_img_object.mode in ['RGB', 'RGBA']:
                bg_image_pil = display_img_object # Already suitable
                logger.info(f"Canvas Prep: Mode {display_img_object.mode} is suitable.")
            elif display_img_object.mode == 'L': # Grayscale to RGB
                logger.info("Canvas Prep: Converting Grayscale (L) image to RGB.")
                bg_image_pil = display_img_object.convert('RGB')
            elif display_img_object.mode == 'P': # Paletted to RGBA (handles transparency better than forcing RGB)
                logger.info("Canvas Prep: Converting Paletted (P) image to RGBA.")
                bg_image_pil = display_img_object.convert('RGBA')
            else: # Other modes (CMYK, YCbCr, etc.) try converting to RGB
                logger.info(f"Canvas Prep: Converting mode {display_img_object.mode} to RGB.")
                bg_image_pil = display_img_object.convert('RGB')

            # Verify conversion success
            if not isinstance(bg_image_pil, Image.Image):
                raise TypeError(f"Image conversion for canvas failed, resulted in type: {type(bg_image_pil)}")
            logger.info(f"Canvas Prep: Final background image ready. Mode: {bg_image_pil.mode}, Size: {bg_image_pil.size}")

        except Exception as prep_err:
            st.error(f"Error: Could not prepare image for the viewer: {prep_err}")
            logger.error(f"Canvas background image preparation FAILED: {prep_err}", exc_info=True)
            bg_image_pil = None # Ensure it's None if prep fails

        # --- Render Canvas only if background image is valid ---
        if isinstance(bg_image_pil, Image.Image):
            ...
            logger.info(f"Canvas Rendering: Calculated dims W={canvas_w}, H={canvas_h}")
            ...
            if canvas_w > 0 and canvas_h > 0:
                ...
                try:
                    ...
                    logger.info(f"Rendering st_canvas with background. Mode: {bg_image_pil.mode}. Initial drawing: {'Set' if initial_drawing else 'None'}")
                    ...
                    # Remind user about console check
                    st.warning("Drawing might be unavailable. Check Browser Console (F12).")
            else:
                # Error if calculated canvas dimensions are invalid
                st.error("Cannot display image viewer: Invalid calculated dimensions.")
                logger.error(f"Invalid canvas dimensions W={canvas_w}, H={canvas_h} prevented rendering.")
        else:
            # Error message if bg_image_pil preparation failed
            # The specific error is already shown by the 'except' block above
            if isinstance(st.session_state.get("display_image"), Image.Image):
                 st.error("Image Viewer Error: Could not display the image. See logs for details.")
            # Don't show placeholder if prep failed, error message is more informative
...
if current_action:
    ...
    try:
        if current_action == "analyze":
            ...
            img_rep = st.session_state.get("display_image") # Use display image for visual consistency
