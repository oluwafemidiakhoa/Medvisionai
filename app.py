...
# --- [ Dependency Checks & Imports section remains largely unchanged ] ---
...
# --- [ Sidebar section remains largely unchanged, including the corrected W/L logic ] ---
...
# --- File Upload Logic --- ADDED MORE LOGGING AND CHECKS ---
        ...
        with st.spinner("🔬 Analyzing and preparing image..."):
            ...
            if st.session_state.is_dicom:
                ...
                try:
                    ...
                    if dicom_dataset:
                        st.session_state.dicom_dataset = dicom_dataset; st.session_state.dicom_metadata = extract_dicom_metadata(dicom_dataset);
                        ...
                        st.session_state.current_display_wc = default_wc; st.session_state.current_display_ww = default_ww;
                        # Generate display image with default W/L
                        temp_display_img = dicom_to_image(dicom_dataset, wc=default_wc, ww=default_ww)
                        # Generate processed image (often needs normalization, maybe grayscale)
                        # Assuming dicom_to_image with normalize=True gives a good base for AI
                        ...
                        if isinstance(temp_display_img, Image.Image):
                            ...
                            # Ensure display image is RGB for Streamlit components
                            if temp_display_img.mode != 'RGB':
                                logger.warning(f"Initial DICOM display image mode is {temp_display_img.mode}. Converting to RGB.")
                                try:
                                    # Handle common conversions explicitly
                                    if temp_display_img.mode == 'L':
                                        # If grayscale, apply colormap or just convert
                                        # For medical images, simple conversion is usually fine
                                        temp_display_img = temp_display_img.convert('RGB')
                                    elif temp_display_img.mode == 'P':
                                        temp_display_img = temp_display_img.convert('RGB')
                                    # Add other conversions if needed (e.g., 'LA', 'RGBA')
                                    else:
                                         temp_display_img = temp_display_img.convert('RGB') # General fallback
                                    ...
                                    temp_display_img = None # Mark as failed
                        ...
                        if isinstance(temp_processed_img, Image.Image):
                             ...
                             # Often processed image for AI should be RGB too
                        ...
                        # Check if BOTH images were successfully created/converted
            ...
            if not st.session_state.is_dicom and not processing_success:
                ...
                try:
                    ...
                    # Force conversion to RGB for consistency
            ...
            if processing_success and isinstance(temp_display_img, Image.Image) and isinstance(temp_processed_img, Image.Image):
                ...
                if processing_success:
                    ...
                    # Clear state if final conversion failed
                    st.session_state.uploaded_file_info = None; st.session_state.display_image = None; st.session_state.processed_image = None; st.session_state.is_dicom = False

            else: # If processing_success is False or images are not PIL Images
                st.error("Image loading failed. Check logs or try a different file.");
                logger.error(f"Processing failed: {uploaded_file.name} (success={processing_success}, display_type={type(temp_display_img)}, proc_type={type(temp_processed_img)})");
...
# --- Column 1: Image Viewer & Metadata --- MODIFIED FOR DEBUGGING ---
with col1:
    ...
    if isinstance(display_img, Image.Image):
        ...
        # Add a button to clear state maybe?
