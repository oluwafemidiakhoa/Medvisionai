...
if not hasattr(st_image, "image_to_url"):
    # Simplified monkey-patch for demonstration
    def image_to_url_monkey_patch(img_obj: Any, *args, **kwargs) -> str:
        if isinstance(img_obj, Image.Image):
            buffered = io.BytesIO()
            format = "PNG" # Default to PNG for simplicity in patch
            try:
...
with st.sidebar:
    ...
    if st.session_state.display_image and st.session_state.is_dicom and st.session_state.dicom_dataset:
        with st.expander("DICOM Window/Level", expanded=False):
            # ... (Keep W/L slider logic exactly as in previous full code version) ...
            ds = st.session_state.dicom_dataset
            pixel_min, pixel_max = 0, 4095
            try:
                arr = ds.pixel_array
            ...
            if abs(new_wc - current_wc) > 1e-3 or abs(new_ww - current_ww) > 1e-3:
                ...
                with st.spinner("Applying Window/Level..."):
                    try:
                        ...
                        # Ensure RGB mode after W/L change
                        if st.session_state.display_image and st.session_state.display_image.mode != 'RGB':
                            st.session_state.display_image = st.session_state.display_image.convert('RGB')
                    except Exception as e:
            ...
            if st.button("Reset W/L", key="reset_wl_btn"):
                ...
                st.rerun()
        st.markdown("---") # Separator even if W/L is not expanded


    # --- 3) Analysis & Interaction Controls (Conditional) ---
    if st.session_state.display_image: # Only show if an image is loaded and valid
        # ... (Keep Initial Analysis Button, Ask AI Question section, Focused Condition Analysis, Confidence & Report sections exactly as in previous full code version) ...
        ...
        disease_select = st.selectbox(
            ...
            options=disease_options_sorted, key="disease_select",
        ...
        with st.expander("📊 Confidence & Report", expanded=True):
            ...
            if st.session_state.get("pdf_report_bytes"):
                ...
                st.download_button(
                    ...
                    key="download_pdf_button", help=f"Download the generated report ({report_filename})"
                )
                st.caption("PDF report data generated successfully.")
            elif st.session_state.last_action == "generate_report_data":
...
# === MAIN PANEL DISPLAYS =====================================================
# =============================================================================
col1, col2 = st.columns([2, 3]) # Image viewer, Results

# --- Column 1: Image Viewer and Canvas ---
with col1:
    st.subheader("🖼️ Image Viewer")

    # --- Check if display_image is valid before using it ---
    # Use isinstance to ensure it's a PIL Image object
    if isinstance(st.session_state.display_image, Image.Image):
        bg_image_pil = st.session_state.display_image
        logger.debug(f"Displaying image in canvas: mode={bg_image_pil.mode}, size={bg_image_pil.size}")

        # Optional: Add st.image here for direct debugging if canvas fails
        # st.image(bg_image_pil, caption=f"Direct Display Check (Mode: {bg_image_pil.mode})", use_column_width='auto')

        # --- Dynamic Canvas Size Calculation ---
        MAX_CANVAS_WIDTH = 600
        MAX_CANVAS_HEIGHT = 550 # Adjusted height
        ...
        canvas_width = max(canvas_width, 150) # Increased minimum size
        canvas_height = max(canvas_height, 150) # Increased minimum size
        logger.debug(f"Calculated canvas dimensions: {canvas_width}x{canvas_height}")


        # --- Display Drawable Canvas ---
        st.caption("Click and drag on the image below to select a Region of Interest (ROI).")
        # Ensure the image passed is definitely RGB before canvas creation (redundant check for safety)
        if bg_image_pil.mode != 'RGB':
             logger.warning(f"Canvas pre-check: Converting image from {bg_image_pil.mode} to RGB.")
        ...
        try:
            canvas_result = st_canvas(
                ...
                stroke_width=2,
                stroke_color="rgba(220, 50, 50, 0.8)", # Slightly different red
                background_image=bg_image_pil, # Pass the verified/converted image
                update_streamlit=True, # Trigger updates on drawing end
                height=int(canvas_height),
                width=int(canvas_width),
                drawing_mode="rect",
                key="canvas",
                # initial_drawing=st.session_state.get("canvas_drawing", None), # May cause issues, keep commented unless needed
            )
             ...
             canvas_result = None # Ensure canvas_result is None on error
            ...
            if last_object["type"] == "rect":
                ...
                if width > 0 and height > 0:
                    ...
                    if st.session_state.roi_coords != new_roi:
                        ...
                        st.rerun() # Update sidebar info/button
                # else: ignore clicks without dragging

        # --- Display DICOM Metadata (Conditional) ---
        if st.session_state.is_dicom and st.session_state.dicom_metadata:
            with st.expander("DICOM Metadata", expanded=False):
                # ... (Keep metadata display logic exactly as in previous full code version) ...
    ...
    else: # If display_image is None or not a valid PIL Image
        st.markdown("---")
        if st.session_state.uploaded_file_info: # Check if upload was ATTEMPTED
             st.warning("⏳ Waiting for image data or processing failed. Check logs if persistent.")
             logger.warning("Image Viewer: st.session_state.display_image is None or not a PIL Image.")
        else:
             st.info("Image will appear here after uploading.")
        # Placeholder visual
        st.markdown("<div style='height: 400px; border: 1px dashed #444; display: flex; align-items: center; justify-content: center; color: #666; font-style: italic;'>Image Display Area</div>", unsafe_allow_html=True)


# --- Column 2: Analysis Results Tabs ---
with col2:
    st.subheader("📊 Analysis & Results")
    # ... (Keep Tabs logic exactly as in previous full code version) ...
    tab_keys = ["Initial Analysis", "Q&A", "Disease Focus", "Confidence"]
    tabs = st.tabs([f" {key} " for key in tab_keys])
    with tabs[0]:
        st.text_area("Overall Findings & Impressions", value=st.session_state.initial_analysis or "No initial analysis performed yet.", height=400, key="output_initial", disabled=True, help="General analysis results for the entire image.")
    with tabs[1]:
        st.text_area("AI Answer", value=st.session_state.qa_answer or "No questions asked yet.", height=250, key="output_qa", disabled=True, help="The AI's answer to your most recent question.")
    ...
    with tabs[2]:
        st.text_area("Disease-Specific Findings", value=st.session_state.disease_analysis or "No focused condition analysis performed yet.", height=400, key="output_disease", disabled=True, help="Analysis results specifically looking for the selected condition.")
    with tabs[3]:
        st.text_area("AI Confidence Estimation", value=st.session_state.confidence_score or "No confidence estimation performed yet.", height=400, key="output_confidence", disabled=True, help="The AI's estimated confidence in its analysis/answers, along with justification.")
...
if current_action:
    ...
    # Ensure an image is loaded and valid before proceeding
    ...
    roi_info_str = " (focused on ROI)" if roi else "" # For user feedback

    try:
        # ... (Keep action handling logic for 'analyze', 'ask', 'disease', 'confidence', 'generate_report_data' exactly as in previous full code version) ...
        ...
        # --- Handle 'ask' action ---
            ...
            if success:
                ...
                # Clearing input text area state can be tricky, often requires form or manual clearing
            else:
                ...
                hf_token_available = bool(os.environ.get("HF_API_TOKEN")) # Example check
        ...
        # --- Handle 'confidence' action ---
            ...
            with st.spinner("Estimating AI confidence..."):
                result = estimate_ai_confidence(st.session_state.history, st.session_state.processed_image, roi)
        ...
        # --- Handle 'generate_report_data' action ---
            ...
            with st.spinner("📄 Generating PDF report data..."):
                img_for_report = st.session_state.display_image # Use the image shown to user
                img_with_roi_for_report = img_for_report
                ...
                if isinstance(img_with_roi_for_report, Image.Image):
                    ...
                    pdf_bytes = generate_pdf_report_bytes(
                        st.session_state.session_id, img_with_roi_for_report, outputs_for_report
                    )
                    if pdf_bytes:
                        ...
                        st.error("Failed to generate PDF report data (backend function returned None).")
                        logger.error("PDF report data generation failed (returned None).")
                else:
                     st.error("Cannot generate report: No valid image available.")
                     logger.error("PDF report generation skipped: No valid image available.")
