...
if uploaded_file is not None and PIL_AVAILABLE:
    try: uploaded_file.seek(0); file_content_hash = hashlib.sha256(uploaded_file.read()).hexdigest()[:16]; uploaded_file.seek(0); new_file_info = f"{uploaded_file.name}-{uploaded_file.size}-{file_content_hash}"
    ...
    if new_file_info != st.session_state.get("uploaded_file_info"):
        logger.info(f"New file upload: {uploaded_file.name} ({uploaded_file.size} bytes)"); st.toast(f"Processing '{uploaded_file.name}'...", icon="⏳"); keys_to_preserve = {"session_id"}; st.session_state.session_id = st.session_state.get("session_id") or str(uuid.uuid4())[:8]
        for key, value in DEFAULT_STATE.items():
            if key not in keys_to_preserve: st.session_state[key] = copy.deepcopy(value) if isinstance(value, (dict, list)) else value
        st.session_state.uploaded_file_info = new_file_info; st.session_state.demo_loaded = False; st.session_state.raw_image_bytes = uploaded_file.getvalue(); file_ext = os.path.splitext(uploaded_file.name)[1].lower(); st.session_state.is_dicom = ( PYDICOM_AVAILABLE and DICOM_UTILS_AVAILABLE and ("dicom" in uploaded_file.type.lower() or file_ext in (".dcm", ".dicom")) )
        with st.spinner("🔬 Analyzing and preparing image..."):
            temp_display_img = None; temp_processed_img = None; processing_success = False
            if st.session_state.is_dicom:
                ...
                try:
                    ...
                    if dicom_dataset:
                        st.session_state.dicom_dataset = dicom_dataset; st.session_state.dicom_metadata = extract_dicom_metadata(dicom_dataset); default_wc, default_ww = get_default_wl(dicom_dataset); st.session_state.current_display_wc = default_wc; st.session_state.current_display_ww = default_ww; temp_display_img = dicom_to_image(dicom_dataset, wc=default_wc, ww=default_ww); temp_processed_img = dicom_to_image(dicom_dataset, wc=None, ww=None, normalize=True)
                        if isinstance(temp_display_img, Image.Image) and isinstance(temp_processed_img, Image.Image): processing_success = True; logger.info("DICOM processed successfully.")
                        else: st.error("Failed to convert DICOM pixels."); logger.error("dicom_to_image invalid return.")
                    else: st.error("Could not parse DICOM."); logger.error("parse_dicom returned None.")
                except pydicom.errors.InvalidDicomError: st.error("Invalid DICOM format."); logger.error("InvalidDicomError."); st.session_state.is_dicom = False
                except Exception as e: st.error(f"DICOM processing error: {e}"); logger.error(f"DICOM error: {e}", exc_info=True); st.session_state.is_dicom = False
            if not st.session_state.is_dicom and not processing_success:
                logger.info("Processing as standard image...");
                try: raw_img = Image.open(io.BytesIO(st.session_state.raw_image_bytes)); processed_img = raw_img.convert("RGB"); temp_display_img = processed_img.copy(); temp_processed_img = processed_img.copy(); processing_success = True; logger.info("Standard image loaded.")
                except UnidentifiedImageError: st.error("Cannot identify image format."); logger.error(f"UnidentifiedImageError: {uploaded_file.name}")
                except Exception as e: st.error(f"Error processing image: {e}"); logger.error(f"Standard image error: {e}", exc_info=True)
            logger.info(f"Image processing completion status: {processing_success}")
            if processing_success and isinstance(temp_display_img, Image.Image) and isinstance(temp_processed_img, Image.Image): st.session_state.display_image = temp_display_img.convert('RGB') if temp_display_img.mode != 'RGB' else temp_display_img; st.session_state.processed_image = temp_processed_img; st.success(f"✅ Image '{uploaded_file.name}' loaded!"); logger.info(f"State updated for: {uploaded_file.name}"); st.rerun()
            else: st.error("Image loading failed."); logger.error(f"Processing failed: {uploaded_file.name} (success={processing_success})"); st.session_state.uploaded_file_info = None; st.session_state.display_image = None; st.session_state.processed_image = None; st.session_state.is_dicom = False

# --- Main Page Content ---

st.markdown("---")
# Main Disclaimer added near top

st.warning( """ **🔴 IMPORTANT: For Research & Educational Use Only 🔴** ... [Disclaimer Text] ... """, icon="⚠️" ) # Shortened for brevity
st.title("⚕️ RadVision AI Advanced: AI-Assisted Image Analysis")
with st.expander("View User Guide & Workflow", expanded=False): st.markdown(""" **Workflow:** ... [Workflow Text] ... """) # Shortened for brevity
st.markdown("---")
...
with col1:
    ...
    if isinstance(display_img, Image.Image):
        st.success("DEBUG: `display_img` object exists.", icon="✅")
        try:
            # --- Using st.image for direct display test ---
            ...
            st.info("Debug Info: Image displayed using `st.image`.", icon="ℹ️")
            ...
            # else: pass
             ...
             st.error(f"Error during image display: {display_e}")
             logger.error(f"Error in display block: {display_e}", exc_info=True)

        # Display ROI info if exists
        ...
        # Display DICOM Metadata if exists
    ...
    elif uploaded_file is not None: st.error("Image preview failed. Check logs."); logger.warning("display_img None despite upload.")
    else: st.info("⬅️ Upload image or use Demo Mode.")
...
if current_action:
    ...
    try:
        ...
        if current_action == "analyze": st.toast("🔬 Analyzing...", icon="⏳");
        with st.spinner("AI analyzing..."): analysis_result = run_initial_analysis(img_for_llm, roi=roi_coords); st.session_state.initial_analysis = analysis_result; st.session_state.qa_answer = ""; st.session_state.disease_analysis = ""; logger.info("Initial analysis done."); st.success("Analysis complete!", icon="✅")
        elif current_action == "ask": question_text = st.session_state.question_input_widget.strip();
        if not question_text: st.warning("Question empty.", icon="❓")
        else: st.toast(f"Asking: '{question_text[:50]}...'"); st.session_state.qa_answer = "";
        ...
        else: primary_error_msg = f"Primary AI failed: {answer}"; st.session_state.qa_answer = primary_error_msg; st.error(primary_error_msg, icon="⚠️"); logger.warning(f"Q&A failed: {answer}"); hf_token = os.environ.get("HF_API_TOKEN") or st.secrets.get("HF_API_TOKEN");
        if HF_MODELS_AVAILABLE and hf_token: st.info(f"Trying fallback: {HF_VQA_MODEL_ID}", icon="🔄");
        with st.spinner(f"Fallback..."): try: fallback_answer, fallback_success = query_hf_vqa_inference_api(img_for_llm, question_text, roi=roi_coords); except Exception as hf_e: fallback_success=False; fallback_answer=f"Err: {hf_e}"; logger.error(f"Fallback err: {hf_e}", exc_info=True)
        if fallback_success: fallback_display=f"**[Fallback: {HF_VQA_MODEL_ID}]**\n\n{fallback_answer}"; st.session_state.qa_answer += f"\n\n---\n\n{fallback_display}"; st.session_state.history.append(("[Fallback] User Q", question_text)); st.session_state.history.append(("[Fallback] AI A", fallback_display)); st.success("Fallback answered.", icon="👍")
        else: fallback_error_msg=f"[Fallback Err - {HF_VQA_MODEL_ID}]: {fallback_answer}"; st.session_state.qa_answer += f"\n\n---\n\n{fallback_error_msg}"; st.error("Fallback failed.", icon="👎"); logger.warning(f"Fallback failed: {fallback_answer}")
        elif HF_MODELS_AVAILABLE and not hf_token: no_token_msg="[Fallback Skip: HF_TOKEN missing]"; st.session_state.qa_answer += f"\n\n---\n\n{no_token_msg}"; st.warning("HF token needed.", icon="🔑")
        else: no_fallback_msg="[Fallback N/A]"; st.session_state.qa_answer += f"\n\n---\n\n{no_fallback_msg}"
        elif current_action == "disease": selected_disease = st.session_state.disease_select_widget;
        if not selected_disease: st.warning("No condition selected.", icon="🏷️")
        else: st.toast(f"🩺 Analyzing for '{selected_disease}'...", icon="⏳");
        with st.spinner(f"AI analyzing..."): disease_result = run_disease_analysis(img_for_llm, selected_disease, roi=roi_coords); st.session_state.disease_analysis = disease_result; st.session_state.qa_answer = ""; logger.info(f"Disease analysis '{selected_disease}' done."); st.success(f"Analysis for '{selected_disease}' complete!", icon="✅")
        elif current_action == "confidence":
            if not current_history: st.warning("Ask a question first.", icon="📊")
            else: st.toast("🧪 Estimating assessment...", icon="⏳");
            with st.spinner("AI assessing..."): assessment_result = run_llm_self_assessment(image=img_for_llm, history=current_history, roi=roi_coords); st.session_state.confidence_score = assessment_result; st.success("Self-assessment complete!", icon="✅")
        elif current_action == "generate_report_data": st.toast("📄 Generating PDF...", icon="⏳"); st.session_state.pdf_report_bytes = None; image_for_report = st.session_state.get("display_image");
        if not isinstance(image_for_report, Image.Image): st.error("No image for report.", icon="🖼️")
        else: final_image_for_pdf = image_for_report.copy().convert("RGB");
        if roi_coords: try: draw=ImageDraw.Draw(final_image_for_pdf); x0,y0,w,h=roi_coords['left'],roi_coords['top'],roi_coords['width'],roi_coords['height']; draw.rectangle([x0,y0,x0+w,y0+h], outline="red", width=max(3,int(min(final_image_for_pdf.size)*0.005))); logger.info("ROI drawn.") except Exception as draw_e: logger.error(f"ROI draw err: {draw_e}", exc_info=True); st.warning("Could not draw ROI.", icon="✏️")
        formatted_history="No Q&A."; if current_history: lines=[]; [lines.append(f"[{qt}]:\n{re.sub('<[^<]+?>','',str(m)).strip()}") for qt,m in current_history]; formatted_history="\n\n---\n\n".join(lines)
        report_data={"Session ID": st.session_state.session_id, "Image Filename": (st.session_state.uploaded_file_info or "N/A").split('-')[0], "Structured Initial Analysis": st.session_state.initial_analysis or "N/P", "Q&A History": formatted_history, "Condition Specific Analysis": st.session_state.disease_analysis or "N/P", "LLM Self-Assessment (Experimental)": st.session_state.confidence_score or "N/P"};
        if st.session_state.is_dicom and st.session_state.dicom_metadata: meta_tags=['PatientName','PatientID','StudyDate','Modality','StudyDescription','InstitutionName']; meta_summary={t: st.session_state.dicom_metadata.get(t,"N/A") for t in meta_tags if t in st.session_state.dicom_metadata}; if meta_summary: lines=[f"{k.replace('PatientName','Pt Name').replace('PatientID','Pt ID').replace('StudyDate','Date').replace('StudyDescription','Desc.')}: {v}" for k,v in meta_summary.items()]; report_data["DICOM Summary"]="\n".join(lines)
        ...
        if pdf_bytes: st.session_state.pdf_report_bytes=pdf_bytes; st.success("PDF ready!", icon="📄"); logger.info("PDF generated."); st.balloons()
        else: st.error("PDF generation failed.", icon="❌"); logger.error("PDF gen returned None.")
