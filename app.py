# main_app.py (Revision - Revert Data URL, Pass PIL Object Directly Again)
...
def safe_image_to_data_url(img: Image.Image) -> str:
    ...
    buffered = io.BytesIO(); format = "PNG" # Default to PNG for broad compatibility & transparency
    try:
        img_to_save = img
...
with col1:
    ...
    if isinstance(display_img_object, Image.Image):
        ...
        try:
            ...
            # Pass display_img_object directly as background_image
            logger.info(f"Rendering st_canvas with PIL background. Image Mode: {display_img_object.mode}, Type: {type(display_img_object)}. Canvas W={canvas_w}, H={canvas_h}. Initial drawing: {'Set' if initial_drawing else 'None'}")

            # **Crucial Check before calling st_canvas**
            if not isinstance(display_img_object, Image.Image):
                 raise TypeError(f"CRITICAL: display_img_object is NOT a PIL Image right before st_canvas call! Type: {type(display_img_object)}")
            if not (isinstance(canvas_w, int) and canvas_w > 0 and isinstance(canvas_h, int) and canvas_h > 0):
                 raise ValueError(f"CRITICAL: Invalid canvas dimensions before st_canvas call! W={canvas_w} ({type(canvas_w)}), H={canvas_h} ({type(canvas_h)})")
            ...
            canvas_result = st_canvas(
                ...
                background_image=display_img_object, # *** Pass the PIL Image object ***
                update_streamlit=True,               # Send drawing updates back
                height=canvas_h,                     # Calculated INT height
                width=canvas_w,                      # Calculated INT width
            ...
            if canvas_result and canvas_result.json_data is not None:
                ...
                if canvas_result.json_data.get("objects"):
                    ...
                    if last_object["type"] == "rect":
                        ...
                        # Uses img_w, img_h calculated earlier from display_img_object
                        if canvas_w <= 0 or canvas_h <= 0: # Avoid division by zero if dims somehow invalid
                             logger.error(f"ROI Scaling Error: Invalid canvas dimensions W={canvas_w}, H={canvas_h}")
                        else:
             ...
             # Catch specific errors related to image/dimensions first
             st.error(f"Image Viewer Error: {ve}")
             logger.error(f"Error preparing for or processing canvas: {ve}", exc_info=True)
        except Exception as canvas_error:
            # Catch other potential errors during st_canvas call or ROI processing
