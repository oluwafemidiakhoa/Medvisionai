...
with st.sidebar:
    ...
    if st.session_state.display_image and st.session_state.is_dicom and st.session_state.dicom_dataset:
        with st.expander("DICOM Window/Level", expanded=False):
            ...
            try: # More robust range calculation
                 ...
                 else: # Handle single value images
                      min_level = pixel_min - 512
                      max_level = pixel_max + 512
                      max_width = 1024
            except: pass # Keep simplified ranges on error
            max_width = max(max_width, 1) # Ensure max_width is at least 1
            ...
            current_ww = max(current_ww, min_width) # Ensure current width is valid
                ...
                with st.spinner("Applying Window/Level..."):
                    try:
                        ...
                        if st.session_state.display_image and st.session_state.display_image.mode != 'RGB':
                             ...
                             logger.info("Converted W/L image to RGB.")
            ...
            if st.button("Reset W/L", key="reset_wl_btn"):
                with st.spinner("Resetting Window/Level..."):
                    try:
                        ...
                        if st.session_state.display_image and st.session_state.display_image.mode != 'RGB':
                            st.session_state.display_image = st.session_state.display_image.convert('RGB')
                            logger.info("Converted reset W/L image to RGB.")
    ...
    if isinstance(st.session_state.display_image, Image.Image): # Check if image is valid before showing controls
...
# --- Column 1: Image Viewer and Canvas (WITH DEBUGGING) ---
with col1:
    st.subheader("🖼️ Image Viewer")

    # --- STEP 1: VERIFY st.session_state.display_image ---
    display_img_object = st.session_state.get("display_image") # Get object safely

    st.write(f"Debug: Type of display_image: `{type(display_img_object)}`") # Print type

    if isinstance(display_img_object, Image.Image):
        st.success("Debug: `st.session_state.display_image` IS a valid PIL Image object.")
        try:
            # Attempt to display directly with st.image for verification
            st.image(display_img_object,
                     caption=f"Direct Debug Display (Mode: {display_img_object.mode}, Size: {display_img_object.size})",
                     use_column_width='always') # Use 'always' for testing layout fit
        except Exception as img_display_error:
            st.error(f"Debug: `st.image` failed to display the image: {img_display_error}")
            logger.error(f"st.image failed: {img_display_error}", exc_info=True)

        # --- If st.image worked, proceed to prepare for canvas ---
        bg_image_pil = display_img_object

        # --- STEP 2: Confirm Image Mode is RGB ---
        if bg_image_pil.mode != 'RGB':
            ...
            try:
                ...
                logger.info("Canvas Prep: Conversion to RGB successful.")
                ...
                bg_image_pil = None # Invalidate if conversion fails

        # --- STEP 3: Check Canvas Dimensions ---
        if bg_image_pil: # Only proceed if we have a valid RGB image
            ...
            if canvas_width > 0 and canvas_height > 0:
                # --- STEP 4 & 5: Display Canvas & Check Console ---
                st.caption("Click and drag on the image below to select a Region of Interest (ROI). (Check Browser Console for errors if blank)")
                try:
                    canvas_result = st_canvas(
                        fill_color="rgba(255, 165, 0, 0.2)",
                        stroke_width=2,
                        stroke_color="rgba(220, 50, 50, 0.8)", # Slightly different red
                        background_image=bg_image_pil, # Should be a valid RGB PIL Image here
                        update_streamlit=True,
                        height=int(canvas_height),
                        width=int(canvas_width),
                        drawing_mode="rect",
                        key="canvas",
                        # initial_drawing=st.session_state.get("canvas_drawing", None), # Keep commented for debugging
                    )
                    # --- Process Drawn ROI (only if canvas loaded) ---
                ...
                st.error("Debug: Calculated canvas dimensions are invalid (<= 0). Cannot draw canvas.")
                logger.error(f"Invalid canvas dimensions calculated: W={canvas_width}, H={canvas_height}")
        else:
             st.error("Debug: Background image is not valid after preparation for canvas.")
             logger.error("Canvas Prep: bg_image_pil became invalid before canvas call.")

    # --- If display_image was NOT a valid PIL Image initially ---
    else:
        st.error("Debug: `st.session_state.display_image` is NOT a valid PIL Image object.")
        logger.error(f"Image Viewer: st.session_state.display_image type is {type(display_img_object)}, expected PIL Image.")
        st.markdown("---")
        if st.session_state.uploaded_file_info:
             st.warning("Image processing likely failed. Check terminal logs for errors during upload.")
        else:
    ...
    # --- Display DICOM Metadata (Conditional, outside the main image check) ---
