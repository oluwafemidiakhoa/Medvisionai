...
try:
    ...
    logger.warning("pylibjpeg not found. For wider DICOM compatibility, please run: pip install pylibjpeg pylibjpeg-libjpeg")
try:
    import gdcm
    logger.info("python-gdcm found.")
except ImportError:
    logger.warning("python-gdcm not found. For improved DICOM compatibility, please run: pip install python-gdcm")
...
try:
    ...
    st.error(f"CRITICAL ERROR importing helpers ({import_error}). Ensure all required .py files are present.")
...
# ------------------------------------------------------------------------------
# <<< --- Page Title & Minimal Disclaimer --- >>>
# ------------------------------------------------------------------------------
st.title("⚕️ RadVision QA Advanced: AI-Assisted Image Analysis")
with st.expander("Usage Guide", expanded=False):
    st.info("This tool is designed for research and informational purposes. Please verify all AI-generated outputs with a certified specialist.")
    st.markdown("**Quick Steps:** 1. Upload image 2. Adjust DICOM W/L (if applicable) 3. Run Analysis 4. Interact via Q&A 5. Generate Report")
st.markdown("---")
...
with st.sidebar:
    ...
    # Additional Sidebar Controls can be added here if needed
...
with col1:
    ...
    if isinstance(display_img, Image.Image):
        ...
        st.markdown("<div style='height: 400px; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center; color: #aaa; font-style: italic;'>Image Display Area<br/>(Upload Required or Processing Failed)</div>", unsafe_allow_html=True)
            ...
            if not isinstance(img_for_report, Image.Image):
                ...
                if roi:
                    try:
                        ...
                        logger.error(f"Error drawing ROI for PDF report: {e}", exc_info=True)
