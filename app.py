...
Features:
  • Refined header, sidebar, and footer using custom CSS.
  • Sectioned navigation (Image Upload & Settings, AI Analysis Actions, Translation & Reports).
  • Unique widget keys to avoid duplicates.
  • A placeholder “Move ROI” button to enable ROI repositioning.
  • Clear interactive feedback through spinners, alerts, and toasts.
"""
...
try:
    ...
    if not hasattr(st_image, "image_to_url"):
        ...
        def image_to_url_monkey_patch(
            image: Any, width: int = -1, clamp: bool = False,
...
with st.sidebar:
    ...
    if nav_option == "Image Upload & Settings":
        ...
        # New "Move ROI" button for interactive ROI repositioning
        if st.button("🛠️ Move ROI", key="move_roi_btn", help="Click to reposition ROI on the image."):
            st.info("Move ROI feature not yet implemented. This is where you would integrate ROI repositioning logic.")
        ...
        if not TRANSLATION_AVAILABLE:
            ...
            # Unique key for the sidebar translation selectbox
            selected_label_reports = st.selectbox(
                "Select source text:",
                options=available_labels,
                key="ts_select_reports"
            )
            if selected_label_reports == "(Enter Custom Text)":
                text_to_translate = st.text_area("Enter text:", value="", height=100, key="ts_custom_reports")
            else:
                text_to_translate = text_options.get(selected_label_reports, "")

            st.text_area("Source Text:", value=text_to_translate, height=80, disabled=True, key="ts_preview_reports")
            col1_ts, col2_ts = st.columns(2)
            with col1_ts:
                src_opts = [AUTO_DETECT_INDICATOR] + sorted(list(LANGUAGE_CODES.keys()))
                src_lang = st.selectbox("From:", src_opts, key="ts_src_reports")
            with col2_ts:
                tgt_opts = sorted(list(LANGUAGE_CODES.keys()))
                tgt_idx = 0
                pref = ["Spanish", "English"]
                [tgt_idx := tgt_opts.index(t) for t in pref if t in tgt_opts]
                tgt_lang = st.selectbox("To:", tgt_opts, index=tgt_idx, key="ts_tgt_reports")
            if st.button("🔄 Translate", key="ts_btn_reports"):
            ...
            if st.session_state.get("translation_result"):
                ...
                st.text_area("Translation:", value=fmt_res, height=150, key="ts_out_reports")
...
# =============================================================================
# FILE UPLOAD & IMAGE PROCESSING LOGIC (only in 'Image Upload & Settings' section)
# =============================================================================
if nav_option == "Image Upload & Settings":
    ...
    with st.container():
        ...
        with tabs[4]:
            ...
            if not TRANSLATION_AVAILABLE:
                ...
                available_labels = [lbl for lbl, txt in text_options.items() if txt or lbl == "(Enter Custom Text)"]
                # Use a unique key for the main tab translation selectbox
                selected_label_tab = st.selectbox(
                    "Select text:",
                    options=available_labels,
                    key="ts_select_tab"
                )
                if selected_label_tab == "(Enter Custom Text)":
                    text_to_translate = st.text_area("Enter text:", value="", height=100, key="ts_custom_tab")
                else:
                    text_to_translate = text_options.get(selected_label_tab, "")
                st.text_area("Source Text:", value=text_to_translate, height=80, disabled=True, key="ts_preview_tab")
                cl1, cl2 = st.columns(2)
                with cl1:
                    src_opts = [AUTO_DETECT_INDICATOR] + sorted(list(LANGUAGE_CODES.keys()))
                    src_lang = st.selectbox("From:", src_opts, key="ts_src_tab")
                with cl2:
                    tgt_opts = sorted(list(LANGUAGE_CODES.keys()))
                    tgt_idx = 0
                    pref = ["Spanish", "English"]
                    [tgt_idx := tgt_opts.index(t) for t in pref if t in tgt_opts]
                    tgt_lang = st.selectbox("To:", tgt_opts, index=tgt_idx, key="ts_tgt_tab")
                if st.button("🔄 Translate", key="ts_btn_tab"):
                    st.session_state.translation_result = None
                    st.session_state.translation_error = None
                    if not text_to_translate.strip():
                        st.warning("No text provided.", icon="☝️")
                ...
                if st.session_state.get("translation_result"):
                    fmt_res = format_translation(st.session_state.translation_result)
                    st.text_area("Translation:", value=fmt_res, height=150, key="ts_out_tab")
                elif st.session_state.get("translation_error"):
                    st.info(f"Error: {st.session_state.translation_error}", icon="ℹ️")
