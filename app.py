...
# --- Setup: Page Config, Logging, Monkey-Patch, Imports (Keep as before) ---
# ... (Code from lines 12 to 114 - Page Config, Logging, Patch, Imports) ...
st.set_page_config(
...
if not hasattr(st_image, "image_to_url"):
    def image_to_url_monkey_patch(img_obj: Any, *args, **kwargs) -> str:
        if isinstance(img_obj, Image.Image):
            ...
            try: img_obj.save(buffered, format=format); img_str = base64.b64encode(buffered.getvalue()).decode("utf-8"); return f"data:image/{format.lower()};base64,{img_str}"
            except Exception as e: logging.error(f"Monkey-patched image_to_url failed: {e}"); return ""
        else: logging.warning(f"Monkey-patched image_to_url: Unsupported type {type(img_obj)}"); return ""
...
try:
    ...
    try: from hf_models import query_hf_vqa_inference_api, HF_VQA_MODEL_ID
        ...
        def query_hf_vqa_inference_api(img: Image.Image, question: str, roi: Optional[Dict] = None) -> Tuple[str, bool]: logging.warning("Using MOCK Hugging Face VQA fallback (hf_models.py not found)."); return f"[Fallback Unavailable] Hugging Face module not found.", False
...
def image_to_data_url(img: Image.Image) -> str:
    buffered = io.BytesIO()
    try: img.save(buffered, format="PNG"); img_str = base64.b64encode(buffered.getvalue()).decode(); return f"data:image/png;base64,{img_str}"
    except Exception as e: logger.error(f"Failed to convert image to data URL: {e}"); return ""

# --- Session State Initialization (Keep as before) ---
# ... (Code from lines 117 to 146 - DEFAULT_STATE dict and loop) ...
DEFAULT_STATE = {
...
# --- Page Title & Disclaimer (Keep as before) ---
# ... (Code from lines 149 to 187 - Title, Disclaimer, Guide) ...
...
# --- Sidebar Controls (Keep as before) ---
# ... (Code from lines 191 to 374 - Sidebar file upload, processing, W/L, actions) ...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
             last_modified = getattr(uploaded_file, 'last_modified', '')
             new_file_info = f"{uploaded_file.name}-{uploaded_file.size}-{last_modified}"
        except Exception: new_file_info = f"{uploaded_file.name}-{uploaded_file.size}"
        ...
        if new_file_info != st.session_state.get("uploaded_file_info"):
            ...
            with st.spinner("🔬 Processing image... Please wait."):
                ...
                if st.session_state.is_dicom:
                    try:
                        ...
                        if ds:
                            ...
                            st.session_state.current_display_wc = wc_default
                            st.session_state.current_display_ww = ww_default
                            ...
                            if temp_display_image and temp_processed_image: logger.info("DICOM images (display & processed) generated successfully."); processing_successful = True
                            else: st.error("DICOM processing failed: Could not generate required image objects from pixel data."); logger.error("dicom_to_image returned None for display and/or processed image.")
                        else: logger.error("parse_dicom returned None.")
                    except Exception as e: st.error(f"An unexpected error occurred during DICOM processing: {e}"); logger.critical(f"DICOM processing pipeline error: {e}", exc_info=True); processing_successful = False
                else: # Standard Image Processing Branch
                    try:
                        ...
                        if img.mode != 'RGB': logger.info(f"Original image mode: {img.mode}. Converting to RGB for display."); temp_display_image = img.convert("RGB")
                        ...
                        processing_successful = True; logger.info("Standard image opened and prepared for display.")
                    except UnidentifiedImageError: st.error(f"Cannot identify image file format for '{uploaded_file.name}'. Please upload JPG, PNG, or DICOM."); logger.error(f"UnidentifiedImageError for file: {uploaded_file.name}"); processing_successful = False
                    except Exception as e: st.error(f"Error processing standard image file '{uploaded_file.name}': {e}"); logger.error(f"Standard image processing error: {e}", exc_info=True); processing_successful = False
                ...
                if processing_successful and isinstance(temp_display_image, Image.Image) and isinstance(temp_processed_image, Image.Image):
                    ...
                    if processing_successful: st.error("❌ Image loading failed unexpectedly after processing. Please check logs.")
                    st.session_state.uploaded_file_info = None; st.session_state.raw_image_bytes = None; st.session_state.display_image = None; st.session_state.processed_image = None
                    st.session_state.dicom_dataset = None; st.session_state.dicom_metadata = {}; st.session_state.current_display_wc = None; st.session_state.current_display_ww = None
    ...
    if st.session_state.is_dicom and st.session_state.dicom_dataset and st.session_state.display_image:
        with st.expander("DICOM Window/Level", expanded=False):
                ...
                if (wc_changed or ww_changed) and wc_current_slider is not None and ww_current_slider is not None:
                    ...
                    with st.spinner("Applying Window/Level..."):
                         ...
                         if new_display_image:
                              ...
                              st.rerun()
                         else: st.error("Failed to apply new Window/Level settings."); logger.error("dicom_to_image returned None when applying new W/L.")
            ...
            except Exception as e: st.error(f"Error in W/L control logic: {e}"); logger.error(f"W/L slider integration error: {e}", exc_info=True)
        st.markdown("---")
    if isinstance(st.session_state.get("display_image"), Image.Image):
        st.subheader("AI Actions")
        if st.button("▶️ Run Initial Analysis", key="analyze_btn", help="Perform a general analysis of the entire image.", use_container_width=True): st.session_state.last_action = "analyze"; st.rerun()
        st.markdown("---")
        st.subheader("❓ Ask AI Question")
        if st.session_state.get("roi_coords"):
            rc = st.session_state.roi_coords; roi_summary = f"L:{rc['left']}, T:{rc['top']}, W:{rc['width']}, H:{rc['height']}"
            st.info(f"✅ ROI Selected: [{roi_summary}]. Question will focus here.")
            if st.button("❌ Clear ROI", key="clear_roi_btn", help="Remove the highlighted region selection.", use_container_width=True): st.session_state.roi_coords = None; st.session_state.canvas_drawing = None; logger.info("ROI cleared by user."); st.rerun()
        else: st.caption("ℹ️ Optionally, draw a rectangle on the image (left panel) to focus your question on a specific region.")
        question_input = st.text_area("Ask about the image or the highlighted region:", height=100, key="question_input_widget", placeholder="e.g., What type of scan is this? Are there any abnormalities in the selected region?", label_visibility="collapsed")
        if st.button("💬 Ask AI", key="ask_btn", use_container_width=True):
            user_question = st.session_state.question_input_widget
            if user_question and user_question.strip(): st.session_state.last_action = "ask"; st.rerun()
        ...
        if st.button("🩺 Run Condition Analysis", key="disease_btn", use_container_width=True):
            ...
            if selected_disease: st.session_state.last_action = "disease"; st.rerun()
        ...
        with st.expander("📊 Confidence & Report", expanded=True):
            ...
            if st.button("📈 Estimate AI Confidence", key="confidence_btn", help="Estimate the AI's confidence based on the conversation history and image.", disabled=not can_estimate_confidence, use_container_width=True): st.session_state.last_action = "confidence"; st.rerun()
            if not can_estimate_confidence: st.caption("Perform an analysis or ask a question first to enable confidence estimation.")
            if st.button("📄 Generate PDF Report Data", key="generate_report_data_btn", help="Compile the session analysis into data ready for PDF download.", use_container_width=True): st.session_state.last_action = "generate_report_data"; st.rerun()
...
with col1:
    ...
    # <<< --- START OF CHANGES for Image Display --- >>>
    # Check if the display_image object exists and is a PIL Image
    if isinstance(display_img_object, Image.Image):
        logger.debug(f"Attempting to display image. Type: {type(display_img_object)}, Mode: {display_img_object.mode}, Size: {display_img_object.size}")
        ...
        try:
            ...
            # ADDED: Explicit check after potential conversion
            if not isinstance(bg_image_pil, Image.Image):
                 raise TypeError("Image object became invalid after RGB conversion attempt.")

        except Exception as convert_err:
             st.error(f"Failed to prepare image for canvas display: {convert_err}")
             logger.error(f"RGB conversion/preparation error for canvas background: {convert_err}", exc_info=True)
             bg_image_pil = None # Ensure it's None if conversion failed
        ...
        if bg_image_pil:
            ...
            if canvas_width > 0 and canvas_height > 0:
                ...
                try:
                    ...
                    # *** THE KEY CHANGE IS HERE: Pass bg_image_pil ***
                    ...
                    if canvas_result and canvas_result.json_data is not None and canvas_result.json_data.get("objects"):
                        # ... (rest of ROI processing logic: last_object, scaling, state update) ...
                    ...
                    # Display specific error related to canvas
                    st.error(f"Error rendering the drawing canvas: {canvas_error}")
             ...
             # Error message already shown if bg_image_pil preparation failed
             st.info("Image prepared for canvas was invalid. Cannot display canvas.")
             logger.error("Canvas cannot be displayed because bg_image_pil is invalid.")
    ...
    # <<< --- END OF CHANGES for Image Display --- >>>


# --- Column 2: Analysis Results Tabs (Keep as before) ---
# ... (Code from lines 467 to 511 - Results Tabs) ...
with col2:
...
# --- Action Handling (Keep full rewritten block as before) ---
# ... (Code from lines 514 to 684 - Action Handling logic) ...
...
if current_action:
    ...
    if not isinstance(processed_image, Image.Image) or not session_id:
        error_msg = f"Cannot perform action '{current_action}': A processed image is required, but not found or the session is invalid."; st.error(error_msg)
        processed_image_type = type(processed_image).__name__ if processed_image else "None"
        logger.error(f"Action '{current_action}' aborted pre-check. Processed image type: {processed_image_type}, Session ID valid: {bool(session_id)}")
        st.session_state.last_action = None; st.stop()
    img_for_llm: Image.Image = processed_image; roi: Optional[Dict[str, int]] = st.session_state.get("roi_coords"); roi_context_str = " (focusing on selected ROI)" if roi else ""
    try:
        ...
        if current_action == "analyze":
            ...
            with st.spinner("AI is analyzing the full image..."): analysis_result = run_initial_analysis(img_for_llm)
            st.session_state.initial_analysis = analysis_result; logger.info("Preliminary analysis action completed successfully."); st.success("Preliminary analysis finished.")
        elif current_action == "ask":
            question = st.session_state.get("question_input_widget", "").strip()
            if not question: st.warning("Please enter a question before clicking 'Ask'."); logger.warning("'Ask' action triggered with empty question input.")
            else:
                st.info(f"❓ Asking the AI your question{roi_context_str}..."); st.session_state.qa_answer = ""
                primary_model_name = "Gemini"
                with st.spinner(f"{primary_model_name} is processing your question{roi_context_str}..."): gemini_answer, success = run_multimodal_qa(img_for_llm, question, st.session_state.history, roi)
                if success:
                    st.session_state.qa_answer = gemini_answer
                    if isinstance(st.session_state.history, list): st.session_state.history.append((question, gemini_answer))
                    else: st.session_state.history = [(question, gemini_answer)]
                    logger.info(f"Multimodal QA successful (Primary: {primary_model_name}) for question: '{question}'{roi_context_str}"); st.success(f"{primary_model_name} answered your question.")
                else:
                    error_message = f"Primary AI ({primary_model_name}) failed to answer. Reason: {gemini_answer}"; st.error(error_message); logger.warning(f"Primary AI ({primary_model_name}) failed for question: '{question}'. Reason: {gemini_answer}")
                    st.session_state.qa_answer = f"**[Primary AI Error]** {gemini_answer}\n\n---\n"
                    hf_token_available = bool(os.environ.get("HF_API_TOKEN")); hf_model_available = bool(HF_VQA_MODEL_ID and HF_VQA_MODEL_ID != "hf_model_not_found" and HF_VQA_MODEL_ID != "unavailable")
                    if hf_token_available and hf_model_available:
                        st.info(f"Attempting fallback using Hugging Face model ({HF_VQA_MODEL_ID})...")
                        with st.spinner(f"Trying fallback AI ({HF_VQA_MODEL_ID}){roi_context_str}..."): hf_answer, hf_success = query_hf_vqa_inference_api(img_for_llm, question, roi)
                        if hf_success:
                            fallback_display = f"**[Fallback Answer ({HF_VQA_MODEL_ID})]**\n\n{hf_answer}"; st.session_state.qa_answer += fallback_display
                            if isinstance(st.session_state.history, list): st.session_state.history.append((question, fallback_display))
                            else: st.session_state.history = [(question, fallback_display)]
                            logger.info(f"HF VQA fallback successful for question: '{question}'{roi_context_str}"); st.success(f"Fallback AI ({HF_VQA_MODEL_ID}) provided an answer.")
                        else:
                            fallback_error_msg = f"Fallback AI ({HF_VQA_MODEL_ID}) also failed. Reason: {hf_answer}"; st.session_state.qa_answer += f"**[Fallback Failed]** {fallback_error_msg}"; st.error(fallback_error_msg); logger.error(f"HF VQA fallback failed for question: '{question}'. Reason: {hf_answer}")
                    else:
                        missing_config = [];
                        if not hf_token_available: missing_config.append("HF Token")
                        if not hf_model_available: missing_config.append("HF Model/Module")
                        fallback_unavailable_msg = f"Fallback AI unavailable (Configuration missing: {', '.join(missing_config)})."; st.session_state.qa_answer += f"**[Fallback Unavailable]** {fallback_unavailable_msg}"; st.warning(fallback_unavailable_msg); logger.warning(f"HF VQA fallback skipped for question '{question}': Configuration missing.")
        elif current_action == "disease":
            selected_disease = st.session_state.get("disease_select_widget")
            if not selected_disease: st.warning("Please select a condition to analyze."); logger.warning("'Disease Analysis' action triggered without a selected condition.")
            else:
                st.info(f"🩺 Analyzing the image for signs of '{selected_disease}'{roi_context_str}...")
                with st.spinner(f"AI is assessing for '{selected_disease}'{roi_context_str}..."): disease_analysis_result = run_disease_analysis(img_for_llm, selected_disease, roi)
                st.session_state.disease_analysis = disease_analysis_result; logger.info(f"Disease-specific analysis completed for '{selected_disease}'."); st.success(f"Analysis for '{selected_disease}' finished.")
        elif current_action == "confidence":
            st.info(f"📊 Estimating AI confidence based on recent interactions{roi_context_str}...")
            current_history = st.session_state.get("history", [])
            if not isinstance(current_history, list) or not current_history: st.warning("Cannot estimate confidence without prior Q&A interaction."); logger.warning("Confidence estimation skipped: No interaction history found.")
            else:
                with st.spinner(f"Calculating confidence score{roi_context_str}..."): confidence_result = estimate_ai_confidence(img_for_llm, current_history)
                st.session_state.confidence_score = confidence_result; logger.info("Confidence estimation action completed."); st.success("AI confidence estimation finished.")
        elif current_action == "generate_report_data":
            st.info("📄 Preparing data for PDF report generation..."); st.session_state.pdf_report_bytes = None
            img_for_report: Optional[Image.Image] = st.session_state.get("display_image"); img_with_roi_drawn = None
            if isinstance(img_for_report, Image.Image):
                ...
                if current_roi and isinstance(current_roi, dict) and all(k in current_roi for k in ['left', 'top', 'width', 'height']):
                    try:
                        ...
                        x0, y0 = int(current_roi['left']), int(current_roi['top']); x1, y1 = x0 + int(current_roi['width']), y0 + int(current_roi['height'])
                        img_w_rep, img_h_rep = img_copy.size; x0 = max(0, min(x0, img_w_rep -1)); y0 = max(0, min(y0, img_h_rep -1)); x1 = max(0, min(x1, img_w_rep)); y1 = max(0, min(y1, img_h_rep))
                        if x1 > x0 and y1 > y0: draw.rectangle([x0, y0, x1, y1], outline="red", width=3); img_with_roi_drawn = img_copy; logger.info("Successfully drew ROI onto image copy for PDF report.")
                        else: logger.warning("ROI dimensions invalid after clamping to image bounds, not drawing."); img_with_roi_drawn = img_for_report
                    except Exception as e: logger.error(f"Failed to draw ROI on report image: {e}", exc_info=True); st.warning("Could not draw the ROI box onto the report image, using original."); img_with_roi_drawn = img_for_report
                else: img_with_roi_drawn = img_for_report
                current_history = st.session_state.get("history", []); full_qa_history = "\n\n".join([f"User Q: {q}\n\nAI A: {a}" for q, a in current_history]) if current_history else "No Q&A interactions recorded."
                outputs_for_report = {"Preliminary Analysis": st.session_state.get("initial_analysis", "Not performed."), "Conversation History": full_qa_history, "Condition-Specific Analysis": st.session_state.get("disease_analysis", "Not performed."), "Last Confidence Estimate": st.session_state.get("confidence_score", "Not estimated.")}
                if st.session_state.get("is_dicom") and st.session_state.get("dicom_metadata"):
                    logger.info("Processing DICOM metadata for report (with filtering)."); filtered_meta_str_list = []
                    PHI_TAGS_TO_EXCLUDE = ["PatientName", "PatientID", "PatientBirthDate", "PatientSex", "OtherPatientIDs", "OtherPatientNames", "PatientAddress", "PatientTelephoneNumbers", "ReferringPhysicianName", "InstitutionName", "InstitutionAddress", "PhysicianOfRecord", "OperatorsName", "IssuerOfPatientID", "PatientBirthTime", "PatientComments", "PerformedProcedureStepStartDate", "PerformedProcedureStepStartTime", "RequestingPhysician", "StudyDate", "SeriesDate", "AcquisitionDate", "ContentDate", "StudyTime", "SeriesTime", "AcquisitionTime", "ContentTime", "AccessionNumber", "StudyInstanceUID", "SeriesInstanceUID", "SOPInstanceUID",]
                    dicom_meta = st.session_state.dicom_metadata
                    for tag_name, tag_value in dicom_meta.items():
                         if tag_name in PHI_TAGS_TO_EXCLUDE: logger.debug(f"Excluding potentially sensitive tag from report: {tag_name}"); continue
                         display_value = ""
                         try:
                             ...
                             elif isinstance(tag_value, pydicom.valuerep.PersonName): display_value = "[Person Name]"
                             else: display_value = str(tag_value).strip()
                             if display_value and len(display_value) < 512: filtered_meta_str_list.append(f"{tag_name}: {display_value}")
                         except Exception as tag_err: logger.warning(f"Error processing report metadata tag '{tag_name}': {tag_err}"); filtered_meta_str_list.append(f"{tag_name}: [Error processing value]")
                    outputs_for_report["DICOM Metadata (Filtered)"] = "\n".join(filtered_meta_str_list) if filtered_meta_str_list else "No applicable metadata found or extracted."; logger.info(f"Included {len(filtered_meta_str_list)} filtered DICOM tags in report data.")
                with st.spinner("🎨 Generating PDF document..."): pdf_bytes = generate_pdf_report_bytes(session_id=session_id, image=img_with_roi_drawn, analysis_outputs=outputs_for_report)
                if pdf_bytes: st.session_state.pdf_report_bytes = pdf_bytes; st.success("✅ PDF report data generated successfully!"); logger.info("PDF report data generation successful.")
                else: st.error("❌ Failed to generate PDF report data. The generation function returned an error. Check logs."); logger.error("PDF report generation failed (generate_pdf_report_bytes returned None or empty).")
            else: st.error("Cannot generate report: The image to include is not valid."); logger.error("PDF generation skipped: No valid PIL Image found in st.session_state.display_image.")
        else: st.warning(f"Action '{current_action}' is not recognized or implemented."); logger.warning(f"Attempted to handle unknown action: '{current_action}'")
    except Exception as e: st.error(f"An unexpected error occurred while trying to perform action '{current_action}'. Please check application logs."); logger.critical(f"Unhandled exception during action '{current_action}': {e}", exc_info=True)
    finally: st.session_state.last_action = None; logger.debug(f"Action '{current_action}' handling finished. Resetting last_action state."); st.rerun()

# --- Footer (Keep as before) ---
# ... (Code from line 687 - Footer) ...
st.markdown("---")
