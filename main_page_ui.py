...
def _render_image_viewer(col) -> None:
    ...
    with col:
        ...
        if _DRAWABLE_CANVAS_AVAILABLE and st_canvas:
            ...
            st.image(img_to_display, caption="Image Preview", use_container_width='auto')
...
def _render_results_tabs(col) -> None:
    ...
    with col:
        ...
        # --- Tab 7: Case History ---
        with tabs[6]:
            st.markdown("**Previous Case Analysis History**")
            st.caption("View and manage previous analyses stored in Google Sheets")
            
            # Import case history functionality
            try:
                from sheets_integration import render_sheet_history, SHEETS_AVAILABLE
                
                # Service account configuration section
                with st.expander("Google Sheets Configuration", expanded=not SHEETS_AVAILABLE):
                    if not SHEETS_AVAILABLE:
                        st.warning("Google Sheets integration requires the 'gspread' package.")
                        st.code("pip install gspread google-auth", language="bash")
                    
                    service_account_exists = bool(os.environ.get("SERVICE_ACCOUNT_JSON"))
                    if not service_account_exists:
                        st.warning("Google Service Account JSON not configured")
                        st.info("To enable case history tracking:")
                        st.markdown("""
                        ...
                        4. Add the JSON contents to Replit Secrets as SERVICE_ACCOUNT_JSON
                        """)
                    else:
                        st.success("Google Service Account configured")
                
                # Render the history table if available
                if SHEETS_AVAILABLE and service_account_exists:
                    if st.button("🔄 Refresh Case History", key="refresh_case_history", use_container_width=True):
                        st.session_state.refresh_case_history = True
                    render_sheet_history()
            except ImportError:
                st.error("Case history feature unavailable - sheets_integration.py not found")
